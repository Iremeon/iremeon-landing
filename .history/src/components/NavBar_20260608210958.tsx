...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${
      isScrolled ? 'bg-bgcolor-300 shadow-lg py-4' : 'py-4'
    }`}>
      <div className="max-w-7xl mx-auto px-4 flex justify-between items-center">
        {/* Logo on the left */}
        <div className="flex-shrink-0">
          <img src="/logo.svg" alt="IremeOn Logo" className="h-10 object-contain" />
        </div>
        ...
        <ul className="hidden lg:flex items-center gap-8">
          {navLinks.map((link) => (
            <li key={link.href}>
              <a
                href={link.href}
                className="text-textcolor-300 hover:text-primarycolor-100 font-medium transition-colors"
                onClick={(e) => {
                  e.preventDefault();
