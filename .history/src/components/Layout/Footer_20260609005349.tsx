  ...
  return (
    <footer className="bg-white py-16 border-t border-bordercolor-100">
      <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8">
        <div>
          ...
          <p className="text-textcolor-200 mb-6 text-sm leading-relaxed">
            Built to Scale, Built to Last. Innovative IT solutions for the
            businesses of today and the future.
          </p>
          ...
          <ul className="space-y-3 text-sm">
            {navLinks.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
                  className="text-textcolor-200 hover:text-primarycolor-100"
                  onClick={(e) => {
                    e.preventDefault();
                    document
      ...
      <div className="mt-12 pt-8 border-t border-bordercolor-100 max-w-6xl mx-auto px-4 flex flex-col md:flex-row justify-between items-center">
        <p className="text-textcolor-200 text-sm">
          &copy; {currentYear} IremeOn Ltd. All rights reserved.
        </p>
        <div className="flex gap-6 mt-4 md:mt-0">
          <Link
            to="/privacy"
            className="text-textcolor-200 hover:text-primarycolor-100 text-sm"
          >
            Privacy Policy
          </Link>
          <Link
